...
public class ClientHandler implements Runnable {
        ...
        try {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (!authenticated && commandId != ProtocolCommands.REGISTER && 
    ...
    private void handleAddEvent(DataInputStream dis, DataOutputStream dos) throws IOException {
    ...
    private void handleNextDay(DataInputStream dis, DataOutputStream dos) throws IOException {
        server.nextDay();
    ...
    private void handleGetQuantity(DataInputStream dis, DataOutputStream dos) throws IOException {
        String product = dis.readUTF();
        int daysLookback = dis.readInt();
        
        long quantity = 0;
        for (int i = 0; i < daysLookback; i++) {
            int day = server.getCurrentDay() - 1 - i;
            if (day < 0) break;
            
            TimeSeries series = server.getTimeSeries(day);
            if (series != null) {
                quantity += series.calculateQuantity(product, daysLookback, server.getCurrentDay());
            }
        }
    ...
    private void handleGetVolume(DataInputStream dis, DataOutputStream dos) throws IOException {
        String product = dis.readUTF();
        int daysLookback = dis.readInt();
        ...
        for (int i = 0; i < daysLookback; i++) {
            ...
            TimeSeries series = server.getTimeSeries(day);
            if (series != null) {
                volume += series.calculateVolume(product, daysLookback, server.getCurrentDay());
            }
        }
    ...
    private void handleGetPriceStats(DataInputStream dis, DataOutputStream dos) throws IOException {
        String product = dis.readUTF();
        int daysLookback = dis.readInt();
        ...
        for (int i = 0; i < daysLookback; i++) {
            ...
            TimeSeries series = server.getTimeSeries(day);
            if (series != null) {
                AggregationResult.PriceStats stats = series.calculatePriceStats(product, daysLookback, server.getCurrentDay());
    ...
    private void handleGetEvents(DataInputStream dis, DataOutputStream dos) throws IOException {
        ...
        List<Event> events = series != null ? series.getEventsForProducts(products) : new ArrayList<>();
    ...
    private void handleWaitSimultaneous(DataInputStream dis, DataOutputStream dos) throws IOException {
        String product1 = dis.readUTF();
        String product2 = dis.readUTF();
        
        boolean result = server.getNotificationManager()
            .waitForSimultaneousSales(product1, product2, 60000, server.getCurrentDay());
    ...
    private void handleWaitConsecutive(DataInputStream dis, DataOutputStream dos) throws IOException {
        String product = dis.readUTF();
        int count = dis.readInt();
        
        String result = server.getNotificationManager()
